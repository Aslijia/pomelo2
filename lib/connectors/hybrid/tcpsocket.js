...
Socket.prototype.close = function () {
  if (!!this.closeMethod && this.closeMethod === 'end') {
    ...
    try {
      ...
      logger.error('socket close failed with destroy', { error: e.message });
    }
  }
};
...
var readHead = function (socket, data, offset) {
  ...
  if (socket.headOffset === socket.headSize) {
    ...
    if (checkTypeData(socket.headBuffer[0])) {
      ...
      dend = data.length;
      logger.debug('close the connection with invalid head message', { remoteAddress: socket._socket.remoteAddress, remotePort: socket._socket.remotePort, data });
      socket.close();
    }
  }
