...
Service.prototype.beforeFilter = function (msg, session, cb) {
  ...
  var next = function (err, resp, opts) {
    ...
    if (typeof handler === 'function') {
      ...
      logger.error('meet invalid before filter, handler or handler.before should be function.', { msg });
...
Service.prototype.afterFilter = function (err, msg, session, resp, cb) {
  ...
  function next(err) {
    ...
    if (typeof handler === 'function') {
      ...
      logger.error('meet invalid after filter, handler or handler.after should be function.', { msg });
