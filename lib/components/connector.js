...
pro.send = function (reqId, route, msg, recvs, opts, cb) {
  logger.debug('connector send message to clinet', { serverId: this.app.serverId, reqId, route, msg, recvs, opts });
...
var bindEvents = function (self, socket) {
  ...
  socket.on('message', function (msg) {
    ...
    if (self.useCrypto) {
      ...
      if (!verified) {
        logger.error('fail to verify the data received from client.', { msg });
        return;
      }
    }
...
var doHandleMessage = function (self, dmsg, session) {
  ...
  if (self.useCrypto) {
    var verified = verifyMessage(self, session, dmsg);
    if (!verified) {
      logger.error('fail to verify the data received from client.', { dmsg });
      return;
    }
  }
...
var getSession = function (self, socket) {
  ...
  logger.debug('session is created with session id', { getServerId: app.getServerId(), sid });
...
var handleMessage = function (self, session, msg) {
  logger.debug('session handleMessage', { serverId: self.app.serverId, id: session.id, msg });
