...
const { getLogger } = require('pomelo2-logger')
...
Module.prototype.monitorHandler = function (agent, msg, cb) {
    ...
    switch (msg.signal) {
        case 'stop':
            ...
            logger.warn('process will stop from conosle module', { serverId: this.app.serverId, params: msg })
            this.app.stop(true)
            break
        case 'list':
        ...
        case 'info':
            utils.invokeCallback(cb, {
                serverId: serverId,
                body: this.app.getCurServer()
            })
            break
        case 'kill':
            utils.invokeCallback(cb, serverId)
            if (agent.type !== 'master') {
                logger.warn('process will kill from console module', { serverId: this.app.serverId, params: msg })
                setTimeout(function () {
                    process.exit(-1)
        ...
        case 'restart':
            ...
            logger.warn('process will restart from console module', { serverId: this.app.serverId, params: msg })
...
var reload = function (app, agent, msg, cb) {
    var successFlag
    var successIds = []
    var serverIds = msg.ids
    var type = msg.type
    ...
    if (!serverIds.length && !!type) {
        ...
        if (!servers) {
            utils.invokeCallback(cb, new Error('reload servers with unknown server type: ' + type))
            return
        }
    ...
    for (var j = 0; j < serverIds.length; j++) {
        agent.request(serverIds[j], module.exports.moduleId, { signal: msg.signal }, function (msg) {
            if (!utils.size(msg)) {
                latch.done()
                return
            }
            ...
            agent.request(id, module.exports.moduleId, { signal: msg.signal }, function (msg) {
                ...
                setTimeout(function () {
                    runServer(app, msg, function (err, status) {
                        if (!!err) {
                            logger.error('restart failed.', { id })
                        } else {
                            successIds.push(id)
                            successFlag = true
                        }
...
var info = function (agent, msg, cb) {
    var sid, record
    var serverInfo = {}
    var count = utils.size(agent.idMap)
    var latch = countDownLatch.createCountDownLatch(count, { timeout: Constants.TIME.TIME_WAIT_COUNTDOWN }, function () {
