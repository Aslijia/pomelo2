...
SessionService.prototype.kickBySessionId = function (sid, reason, cb) {
  ...
  if (session) {
    // notify client
    session.closed(typeof reason === 'object' ? JSON.stringify(reason) : reason);
    process.nextTick(function () {
      utils.invokeCallback(cb);
    });
  } else {
...
SessionService.prototype.sendMessage = function (sid, msg) {
  ...
  if (!session) {
    logger.debug('Fail to send message for non-existing session ', { sid, msg });
    return false;
  }
...
SessionService.prototype.sendMessageByUid = function (uid, msg) {
  ...
  if (!sessions) {
    logger.debug('fail to send message by uid for non-existing session.', { uid, msg });
    return false;
  }
...
const Session = function (sid, frontendId, socket, service) {
  ...
  socket.on('heartbeat', this.emit.bind(this, 'heartbeat'));
};
...
Session.prototype.closed = function (reason) {
  logger.debug('session is closed with session id', { frontendId: this.frontendId, id: this.id });
