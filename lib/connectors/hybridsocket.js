 ...
 */
var Socket = function (id, socket,options) {
  ...
  if (!socket._socket) {
    this.remoteAddress = {
      ip: (options && options['x-real-ip']) || socket.address().address,
...
Socket.prototype.sendRaw = function (msg) {
  ...
  this.socket.send(msg, { binary: true }, function (err) {
    if (!!err) {
      logger.error('websocket send binary data failed', { error: err.message, stack: err.stack });
      return;
    }
  });
};
