...
Application.init = function (opts) {
    ...
    logger.info('application inited', { server: this.getCurServer() })
}
 ...
 *
 * @param {Object} logger pomelo2-logger instance without configuration
 *
 * @memberOf Application
 */
Application.configureLogger = function (logger) {
    if (process.env.POMELO_LOGGER !== 'off') {
        var log4js = this.getConfigBasePath(Constants.FILEPATH.LOG)
        if (log4js) {
            logger.configure(log4js, { serverId: this.serverId, base: base })
        } else {
            logger.error('logger file path configuration is error.', { message: 'log4js.json not found' })
        }
    }
}
...
Application.getConfigBasePath = function (val) {
    var globalPath = path.join(os.homedir(), '.pomelo2', this.name || '', val)
    var originPath = path.join(this.getBase(), val)
    var presentPath = path.join(this.getBase(), Constants.FILEPATH.CONFIG_DIR, this.env, path.basename(val))
...
Application.loadConfigBaseApp = function (key, val, reload) {
    const realpath = this.getConfigBasePath(val)
    if (!realpath) {
        logger.error('invalid configuration with file path', { key, val })
        return
    }
    var pfile = require(realpath)
    this.set(key, pfile[this.env] ? pfile[this.env] : pfile)
        ...
        var startUp = function () {
            appUtil.optComponents(self.loaded, Constants.RESERVED.START, function (err) {
                ...
                if (err) {
                    ...
                    logger.info('enter after start', { server: self.getCurServer() })
                    self.afterStart(cb)
                }
            })
        }
...
Application.afterStart = function (cb) {
    ...
    appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, function (err) {
        ...
        if (!err) {
            logger.info('finish start', { server: self.getCurServer() })
        }
        if (!!afterFun) {
...
Application.stop = function (force) {
    if (this.state > STATE_STARTED) {
        logger.warn('application is not running now.', { srverId: this.serverId, state: this.state })
        return
    }
...
Application.use = function (plugin, opts) {
    if (!plugin.components) {
        logger.error('invalid components, no components exist', { plugin, opts })
        return
    }
...
Application.addCrons = function (crons) {
    if (!crons || !crons.length) {
        logger.warn('crons is not defined.', { crons })
        return
    }
...
Application.removeCrons = function (crons) {
    if (!crons || !crons.length) {
        logger.warn('ids is not defined.', { crons })
        return
    }
