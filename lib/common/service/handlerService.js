...
var forwardLogger = require('pomelo2-logger').getLogger('forward-log', __filename);
/**
 * Handler service.
...
Service.prototype.handle = function (routeRecord, msg, session, cb) {
  ...
  var callback = function (err, resp, opts) {
    if (self.enableForwardLog) {
      ...
      forwardLogger.info('forward log', log);
    }
  ...
  if (!Array.isArray(msg)) {
    ...
    if (promise && promise.catch) {
      promise.catch((err) => {
        logger.error('handler has error', { method, msg, error: err.message, stack: err.stack });
        callback(null, { code: 505, tipd: err.message });
      });
    }
  } else {
    msg.push(session);
