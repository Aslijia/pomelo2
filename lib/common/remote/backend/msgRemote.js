...
var logger = require('pomelo2-logger').getLogger('forward-log', __filename);
/**
...
Remote.prototype.forwardMessage = function (msg, session, cb) {
  ...
  logger.debug('remote handle message', { serverId: this.app.serverId, msg });
...
Remote.prototype.forwardMessage2 = function (route, body, aesPassword, compressGzip, session, cb) {
  ...
  if (!sessionService) {
    logger.error('backend session component not enable', { serverId: this.app.serverId });
    utils.invokeCallback(cb, new Error('backend sesssion component not enable'));
    return;
  }
  ...
  logger.debug('backend server handle message', { serverId: this.app.serverId, msg });
