...
var protocol = require('pomelo2-protocol');
...
Socket.prototype.close = function () {
  if (!!this.closeMethod && this.closeMethod === 'end') {
    ...
    try {
      ...
      logger.error('socket close with destroy', { error: e.message, stack: e.stack });
    }
  }
};
...
var readHead = function (socket, data, offset) {
  ...
  if (socket.headOffset === socket.headSize) {
    ...
    if (checkTypeData(socket.headBuffer[0])) {
      ...
      dend = data.length;
      logger.error('close the connection with invalid head message', { remoteAddress: socket._socket.remoteAddress, remotePort: socket._socket.remotePort, data });
      socket.close();
    }
