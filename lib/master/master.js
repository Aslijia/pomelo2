...
var crashLogger = require('pomelo2-logger').getLogger('crash-log', __filename);
var adminLogger = require('pomelo2-logger').getLogger('admin-log', __filename);
var admin = require('pomelo2-admin');
...
Server.prototype.start = function (cb) {
  ...
  this.masterConsole.on('admin-log', function (log, error) {
    if (error) {
      adminLogger.error('master logger', { log });
    } else {
      adminLogger.info('master logger', { log });
    }
  });
};
